...
function ViewEntry(){
    ...
    async function getTargetEntry(){
        axios.post("http://127.0.0.1:8080/api/v1/entries/viewOne/"+id, {}, {headers:{Authorization:`Bearer ${user.accessToken}`}})
        .then((result)=>{
    ...
    async function handleActualDelete(){
        axios.put("http://127.0.0.1:8080/api/v1/entries/delete/"+id, {}, {headers:{Authorization:`Bearer ${user.accessToken}`}}).then(()=>{
            navigate("/allEntries");
        })
    }
    return(
